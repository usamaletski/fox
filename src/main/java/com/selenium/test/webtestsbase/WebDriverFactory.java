...
public class WebDriverFactory {
    ...
    public static void startBrowser(boolean isLocal) throws MalformedURLException {
        if (driver == null) {
            ...
            if (!isLocal) {
                driver = new RemoteWebDriver(new URL("http://127.0.0.1:4723/wd/hub"), CapabilitiesGenerator.getDefaultCapabilities(browser));
            } else {
                switch (browser) {
                    case FIREFOX:
                        System.setProperty("webdriver.firefox.bin", TestsConfig.getConfig().getFirefoxBinUrl());
